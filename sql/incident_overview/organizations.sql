...
org_activity AS (
  -- Source 1: Organizations that created worksites (from omni)
  -- NOTE: is_daily_last is NULL for most rows, so we don't filter by it
  SELECT DISTINCT
  ...
  -- Source 2: Organizations that claimed work types (from omni)
  SELECT DISTINCT
    omni.the_date AS date_active,
  ...
  -- Source 3: Organizations whose users updated work type statuses
  SELECT DISTINCT
  ...
  -- Source 4: Organizations whose users took inbound calls
  SELECT DISTINCT
