...
worksites_in_period AS (
  ...
  WHERE incident_id = ANY(string_to_array('175', ',')::int[])
    ...
    AND RIGHT(COALESCE(home_local_division_name, home_city_name), 2) = 'STATE_PLACEHOLDER'  -- State filter
    AND invalidated_at IS NULL
),
end_of_period_snapshot AS (
  -- Get the final snapshot for each worksite's work types in this state
